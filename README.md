...
- **Primary model trained from NASA ACES aero-piston telemetry**:
  ...
  - **Performance Envelope (Domain-Dependent)**:

    | Metric | Synthetic Fault Trajectories | Real NASA ACES Flights |
    |--------|------------------------------|------------------------|
    | Accuracy | 99.81% | 89.47% |
    | Balanced Accuracy | 99.20% | 89.29% |
    | Critical Recall | 99.42% | 95.68% |
    | Critical Precision | 98.84% | 63.88% |
    | Critical F1 | 0.991 | 0.766 |
    | Anomaly AUROC | 0.991 | 0.960 |
    | False Alarm Rate | 0.4% | 5.79% |

    > **Note**: Synthetic metrics are evaluated on deterministic fault-injection trajectories generated by the AeroPulse physics engine. Real metrics are evaluated on held-out NASA ACES Altus II flights which contain zero engine failures — all 'critical' labels on real data come from transient flight envelope exceedances.
...
```text
NASA ACES (173,878 rows, 14 flights) ──► Altus II operational/mechanical flight telemetry (Rotax 914 Turbo piston engine)
...
1. **NASA ACES**: Real Altus II operational/mechanical flight telemetry (four-cylinder Rotax 914 Turbo piston engine) used for operational-envelope health classification and contextual validation. **Does NOT contain run-to-failure degradation trajectories or target-engine RUL ground truth** (all flights completed safely).
...
- **Live Production URL**: [https://aeropulse-test.vercel.app](https://aeropulse-test.vercel.app)
- **FastAPI Interactive API Docs**: [https://aeropulse-test.vercel.app/docs](https://aeropulse-test.vercel.app/docs)
...
# Run the complete test suite (308 passed)
pytest -v
...
- **Verified Test Suite**: **308 passed, 0 failed, 0 skipped, 0 xfailed** across all test modules.
